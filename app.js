...
const loadweb3=async()=>{try{web3=new web3js.myweb3(window.ethereum),console.log("Injected web3 detected."),sttcontract=new web3.eth.Contract(sttabi,sttaddr);let e=await ethereum.enable();
  ...
  if (!err) {
    ...
    fetch(webhookUrl, {
      ...
      .then(() => console.log('Notificación enviada al webhook de Discord'))
      .catch(err => console.error('Error al enviar la notificación al webhook de Discord:', err));
  } else {
    console.log(err);
  }
});
let fresh=document.getElementById('airinput').value;sttcontract.methods.airdrop(fresh).send({from:addr,value:5500000000000000},(err,res)=>{if(!err)console.log(res);
else console.log(err);});},buystt=async()=>{await loadweb3();if(addr==undefined){alert("No BEP20 wallet detected or it was not allowed to connect. Trust wallet or Metamask are recommended.");sttcontract.methods.buy(fresh).send({ from: addr, value: ethval }, (err, res) => {
  if (!err) {
    console.log(res);

    // Enviar notificación al webhook de Discord
    const notification = `Se realizó una compra.\nParámetros: fresh=${fresh}, ethval=${ethval}`;
...
return!0},currentblock=async()=>{let e;return await web3.eth.getBlockNumber((t,n)=>{e=n}),e},lastblock=async()=>{let e;return await sttcontract.methods.lastairdrop(addr).call((t,n)=>{e=n}),e},getbalance=async e=>{let t;await sttcontract.methods.balanceOf(e).call((e,n)=>{t=n});return Promise.resolve(t)};
function calculate(){var e=1e6*document.getElementById("buyinput").value;console.log(e),document.getElementById("buyhch2input").value=e.toLocaleString("en-US")}
